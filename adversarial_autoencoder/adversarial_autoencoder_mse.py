...
model_name = "Adversarial_autoencoder_MSE"
...
class AAE() :
    ...
    def encoder_model(self, encoder_input) :      
        ...
        latent_mean = Dense(self.latent_size, name = 'latent_mean')(encoder_layer3)
        latent_var = Dense(self.latent_size, name = 'latent_var')(encoder_layer3)
    ...
    def discriminator_model(self, random_input) :
        ...
        dis_output = Dense(1, name = 'discriminator_output')(layer5_activation)
    ...
    def train(self, train_data_path, test_data_path, load = False, disc_weights_path = 0, aae_weights_path = 0) :       
        ...
        parallel_discriminator.compile(optimizer = 'Adam', loss = 'mse', metrics = [metrics.mse])
                                       
        parallel_aae = ModelMGPU(self.adversarial_autoencoder, gpus = 2)
        parallel_aae.compile(loss = ['mse', 'mse'], loss_weights = [0.999, 0.001], optimizer = 'Adam',
        ...
        for epoch in range(self.nb_epoch) :
            ...
            for i in range(interval) :
                ...
                print("%d D [fake data] => loss : %f, mse : %f    [real data] => loss : %f, mse : %f"%(i, 
                    fake_loss[0], fake_loss[1], real_loss[0], real_loss[1]))
                ...
                print("   G [loss: %f, ae_mse : %f, disc_mse : %f]\n"%(aae_loss[0], aae_loss[3], aae_loss[6]))
                
            # weights 저장
            parallel_discriminator.save_weights(self.disc_weights_path + "/epoch_{:d}".format(epoch) +
                                    "__disc-mse_{:.4f}".format(float(discriminator_loss[1])) +
                                    "__disc-loss_{:.4f}_".format(float(discriminator_loss[0])) + ".hdf5")
            parallel_aae.save_weights(self.aae_weights_path + "/epoch_{:d}".format(epoch) +
                                    "__AAE-loss_{:.4f}".format(float(aae_loss[0])) +
                                    "__AEE-mse_{:.4f}_".format(float(aae_loss[3])) + ".hdf5")
